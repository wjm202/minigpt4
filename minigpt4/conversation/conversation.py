...
from paddlenlp.generation.stopping_criteria import StoppingCriteria, StoppingCriteriaList,StoppingCriteriaSub   
import dataclasses
...
class Chat:
    ...
    def answer(self, conv, img_list, max_new_tokens=300, num_beams=1,
        ...
        with self.maybe_autocast():
            ...
            # import numpy as np
            # embs=paddle.to_tensor(np.load('/paddle/minigpt4/embeds.npy'))
            ...
            outputs = self.model.llama_model.generate(inputs_embeds=embs,
                max_new_tokens=max_new_tokens, stopping_criteria=self.stopping_criteria, num_beams=num_beams, do_sample=True,
                min_length=min_length, top_p=top_p, repetition_penalty=repetition_penalty, length_penalty=length_penalty, temperature=temperature)
            # outputs = self.model.llama_model.generate(inputs_embeds=embs,
            #     max_new_tokens=max_new_tokens, stopping_criteria=self.stopping_criteria, num_beams=num_beams, do_sample=True,
            #     min_length=min_length, top_p=top_p, repetition_penalty=repetition_penalty, length_penalty=length_penalty, temperature=temperature)
    ...
    def get_context_emb(self, conv, img_list):
        ...
        seg_tokens = [self.model.llama_tokenizer(seg, return_tensors='pd',
